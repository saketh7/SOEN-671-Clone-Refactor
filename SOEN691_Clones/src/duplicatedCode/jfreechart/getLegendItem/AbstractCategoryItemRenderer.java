...
public abstract class AbstractCategoryItemRenderer extends AbstractRenderer
	...
	protected LegendItem getLegendItemExtracted(int series, int datasetIndex, boolean arg0, boolean arg1,
			boolean thisUseFillPaint, boolean thisDrawOutlines, boolean thisUseOutlinePaint) {
		CategoryPlot cp = getPlot();
		if (cp == null) {
			return null;
		}
		if (isSeriesVisible(series) && isSeriesVisibleInLegend(series)) {
			...
			Stroke outlineStroke = lookupSeriesOutlineStroke(series);
			LegendItem result = new LegendItem(label, description, toolTipText, urlText, arg0, shape,
					getItemShapeFilled(series, 0), fillPaint, shapeOutlineVisible, outlinePaint, outlineStroke, arg1,
					new Line2D.Double(-7.0, 0.0, 7.0, 0.0), getItemStroke(series, 0), getItemPaint(series, 0));
